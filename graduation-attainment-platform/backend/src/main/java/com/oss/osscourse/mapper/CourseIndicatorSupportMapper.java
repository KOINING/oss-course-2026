...
public interface CourseIndicatorSupportMapper extends BaseMapper<CourseIndicatorSupport> {
    ...
    @Select({
            ...
            "UNION",
            "SELECT tc.grade_year AS grade_year",
            "FROM teaching_class tc",
            "JOIN course_major cm2 ON cm2.course_id = tc.course_id AND cm2.grade_year = tc.grade_year",
            "<if test='majorId != null'>WHERE cm2.major_id = #{majorId}</if>",
            "UNION",
            "SELECT s.enrollment_year AS grade_year FROM student s",
            "<if test='majorId != null'>WHERE s.major_id = #{majorId}</if>",
            ") years",
